...
import { Transaction, TransactionType, FrequencyType } from '../types';
    ...
    <div className="flex flex-col h-screen bg-app-bg text-app-text">
      <form onSubmit={handleSubmit} className="flex flex-col h-full">
          ...
          <div className="text-center space-y-2">
            ...
            <div className="flex items-center justify-center">
              ...
              <input
                ...
                placeholder="0.00"
                className="w-full max-w-[200px] bg-transparent text-center text-5xl font-bold placeholder-app-muted/30 focus:outline-none"
                autoFocus={!editId}
                required
              />
            </div>
        ...
        <footer className="fixed bottom-0 left-0 right-0 p-4 bg-app-bg/80 backdrop-blur-xl border-t border-app-border">
          <button
            type="submit"
            disabled={!amount || parseFloat(amount) <= 0 || !categoryId || addTransactionMutation.isPending || updateTransactionMutation.isPending || addRecurringMutation.isPending}
            className="w-full h-14 rounded-2xl text-lg font-bold bg-app-primary text-white disabled:bg-app-elevated disabled:text-app-muted transition-all"
          >
            {addTransactionMutation.isPending || updateTransactionMutation.isPending || addRecurringMutation.isPending ? 'Guardando...' : (
              !amount ? 'Ingresa un monto' :
                !categoryId ? 'Selecciona categoría' :
